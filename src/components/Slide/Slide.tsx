...
import Charecter from '@/../public/Home/Charecters.png'
const Slide = () => {
    // ${!isActive && 'scale-75'}
    return (
        <div className={`relative mx-auto`} style={{transition: 'all 0.3s ease-in'}}>
            <div className={`w-full h-[77%] bottom-0 rounded-xl absolute bg-black transition-all`}/>
            <Image src={Banner} alt={"Banner"} width={1200} height={479}
                   className={`absolute -bottom-0 -z-10`} quality={100}/>
            <Image src={Charecter} alt={"Charecters"} width={910} height={534}
                   className={`mx-auto`} style={{transition: 'all 0.3s ease'}} priority={true} loading={'eager'} quality={100}/>
            <div
